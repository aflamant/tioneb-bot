    ...
    .then(connection => {
      const dispatcher = connection
        ...
        .on('start', () => {
          ...
          return message.guild.channels.find(channel => channel.name === 'general').send(`:musical_note:   Now playing: ${queue[0].title} - ${queue[0].duration}`);
        })
        .on('finish', () => {
          ...
          if (queue.length >= 1) {
            ...
            isPlaying = false;
            message.guild.channels.find(channel => channel.name === 'general').send(`:musical_note:   Fin de la playlist atteinte`);
